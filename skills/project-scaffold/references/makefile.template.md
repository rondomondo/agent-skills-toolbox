...
- `install` depends on `check-venv` -- venv must be active before pip install

## GitHub SSH target

Include `github-check` only if the user provides an SSH key path during Step 5.

## CI target

`ci` runs `format lint typecheck test` in sequence. If any step fails, the pipeline stops.
